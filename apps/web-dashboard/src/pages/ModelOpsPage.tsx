    ...
    <div className="flex flex-col gap-6">
      ...
      {showRollbackModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm animate-[fadeIn_.2s_ease_both]">
          <div className="w-[440px] rounded-2xl border border-amber-500/30 bg-zinc-950 p-6 shadow-[0_0_40px_rgba(245,158,11,0.15)]">
      ...
      <SectionHeader
        ...
        subtitle="Manage champion deployments, monitor feature drift, and execute challenger promotions"
        action={
          <button
            type="button"
            onClick={() => void refreshOps()}
            className="flex items-center gap-2 rounded-lg border border-zinc-700 bg-zinc-800/60 px-3 py-1.5 text-xs text-zinc-300 hover:text-cyan-200 transition-colors"
          >
            ↻ Sync Model State
            <span className="text-[10px] text-zinc-500">last checked {relativeTime(new Date().toISOString())}</span>
          </button>
        }
      />

      {ops.loading ? (
         <div className="animate-[shimmer_2s_infinite] bg-[linear-gradient(to_right,#00000000,#ffffff0a,#00000000)] bg-[length:400%_100%] space-y-6">
            ...
            <div className="h-[400px] rounded-xl bg-zinc-900/50" />
         </div>
      ) : (
        <>
          {/* Schedule KPIs */}
          ...
          <div className="grid gap-6 lg:grid-cols-12">
            ...
            <Panel className="lg:col-span-6 flex flex-col" glow="violet">
              <div className="mb-5 flex items-center justify-between">
                <h3 className="text-base font-semibold text-zinc-100">Performance Head-to-Head</h3>
                {prDelta !== null && (
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                             ...
                             <span className={`rounded px-1.5 py-0.5 text-[9px] uppercase font-bold tracking-widest ${isChamp ? 'bg-violet-500 text-white' : 'bg-zinc-800 text-zinc-400'}`}>{tag}</span>
                          </div>
                          <p className="font-mono text-xs text-zinc-500 truncate max-w-[200px] mb-1">{m.version}</p>
                          <p className="text-[10px] text-zinc-600">Updated {relativeTime(m.updated_at ?? new Date().toISOString())}</p>
                        </div>
                        <Badge value={m.status} size="md" />
                      </div>
                      <div className="mt-4 space-y-1">
                        <div className="flex justify-between items-end">
                          <p className="text-[10px] font-semibold uppercase tracking-widest text-zinc-500">Precision-Recall AUC</p>
            ...
            <Panel className="lg:col-span-3 flex flex-col" glow="amber">
              <div className="mb-4">
                <h3 className="text-base font-semibold text-zinc-100 mb-1">Feature Drift Monitor</h3>
                <p className="text-xs text-zinc-500">Continuous population stability</p>
              </div>
              
              {drift && psiSeverity ? (
                <div className="flex flex-col gap-4 flex-1">
                  <div className={`rounded-xl border p-4 text-center shadow-inner ${psiSeverity.cls}`}>
                     <p className="text-[10px] font-bold uppercase tracking-widest mb-1 opacity-80">Global PSI Status</p>
                     <p className="text-xl font-black tracking-wide">{psiSeverity.label}</p>
                     <p className="text-[10px] uppercase font-semibold mt-1 opacity-80">{psiSeverity.desc}</p>
                  </div>
                  ...
                  <div className="mt-auto pt-4 border-t border-zinc-800/80">
                    <p className="text-[10px] font-semibold uppercase tracking-widest text-zinc-500 mb-2">Automated Policy</p>
            ...
            <Panel className="lg:col-span-3">
              <h3 className="mb-1 text-base font-semibold text-zinc-100">Operations Control</h3>
              <p className="mb-5 text-xs text-zinc-500">Execute manual overrides</p>
              ...
              <div className="space-y-3">
                ...
                >
                  ...
                  <span className="text-[10px] text-emerald-500/70 group-hover:text-emerald-400 transition-colors">Replace active champion</span>
                </button>
                ...
                <button
                  ...
                  className="group flex w-full flex-col items-center justify-center gap-1 rounded-xl border border-violet-500/30 bg-violet-500/10 p-3 transition-all hover:bg-violet-500/20 hover:border-violet-500/50 disabled:opacity-50"
                >
                  ...
                  <span className="text-[10px] text-violet-500/70 group-hover:text-violet-400 transition-colors">Generate new challenger</span>
                </button>
                ...
                <button
                  ...
                  className="group flex w-full flex-col items-center justify-center gap-1 rounded-xl border border-amber-500/30 bg-zinc-950 p-3 transition-all hover:bg-amber-500/10 hover:border-amber-500/50 disabled:opacity-50"
                >
                  ...
                  <span className="text-[10px] text-zinc-500 group-hover:text-amber-500/70 transition-colors">Revert to stable</span>
                </button>
              </div>
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            ...
            <Panel className="flex flex-col">
              <h3 className="mb-4 text-base font-semibold text-zinc-100 flex items-center gap-2">
                 Optimization Log
                 {history.length > 0 && <span className="bg-zinc-800 text-zinc-400 text-[10px] px-2 py-0.5 rounded-full">{history.length} cycles</span>}
              </h3>
              <div className="flex-1 overflow-x-auto border border-zinc-800/80 rounded-xl bg-zinc-950/40">
                <table className="w-full text-left text-xs whitespace-nowrap">
                  <thead className="bg-zinc-900 border-b border-zinc-800 text-[10px] uppercase font-semibold tracking-wider text-zinc-500">
                    <tr>
                  ...
                  <tbody className="divide-y divide-zinc-800/50">
                    {history.map((item) => (
                      <tr key={item.id} className="hover:bg-zinc-800/30 transition-colors group">
                        <td className="p-3 font-mono text-[11px] text-zinc-500 group-hover:text-zinc-400">{item.id.slice(0, 8)}...</td>
                        <td className="p-3">
                        ...
                        <td className="p-3">
                          <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-widest ${item.promoted ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-zinc-800 text-zinc-500 border border-zinc-700'}`}>
            ...
            <Panel className="flex flex-col">
              <h3 className="mb-4 text-base font-semibold text-zinc-100 flex items-center gap-2">
                 Compiled Artifacts
                 {artifacts.length > 0 && <span className="bg-zinc-800 text-zinc-400 text-[10px] px-2 py-0.5 rounded-full">{artifacts.length} blobs</span>}
              </h3>
              <div className="grid gap-3 sm:grid-cols-2">
                {artifacts.map((a) => (
                  <div key={a.name} className="group rounded-xl border border-zinc-800/80 bg-zinc-950 p-4 transition-colors hover:border-zinc-700 hover:bg-zinc-900 cursor-pointer">
                    ...
                    <p className="font-mono text-[10px] text-zinc-500 break-all mb-3 line-clamp-1">{a.path}</p>
                    <p className="text-[10px] font-medium text-zinc-600">Last touched: {relativeTime(a.updated_at)}</p>
                  </div>
                ))}
              </div>
            </Panel>
